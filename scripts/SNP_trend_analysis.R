...
setwd ("E:/Malaria_Project/New documents")
...
snp_data_long <- all_data %>%
  ...
  mutate(
    allele = ifelse(allele == "-", "Untyped", allele)  # 👈 Change legend label
  )
...
# --- 5. Plot allele frequency trends (one file per SNP) ---
# --- 5. Plot allele frequency trends (one file per SNP) ---
...
for (snp_name in unique_snps) {
  ...
  p <- ggplot(snp_data, aes(x = factor(year), y = percent, fill = allele)) +
    ...
    theme(
      ...
      panel.grid.major = element_line(color = "gray50"),  # 👈 now it's inside theme()
      panel.grid.minor = element_blank()
    )
  
  # Save each SNP plot as a separate file
